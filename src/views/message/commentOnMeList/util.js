...
export const commentToReply = comment => {
    ...
    if (comment.level == 1 || comment.level == 2) {
        reply.nick = comment.user_detail_info[0] && comment.user_detail_info[0].nick_name ? `${comment.user_detail_info[0].nick_name}` : ''
        reply.date = comment.created_at ? `${moment(comment.created_at).format('YYYY-MM-DD HH:mm')}` : ''
        reply.avatar = comment.user_detail_info[0] && comment.user_detail_info[0].avatar ? `${comment.user_detail_info[0].avatar}` : 'personalicon'
